...
if(empty($desc_fields))
{
	...
	$desc_fields .= '<img class="action-text2" src="./images/add.png" alt="" onclick="document.mainform.desc.rows+=4" onmouseover="Tip(\'Add 4 rows\')" onmouseout="UnTip()" />';
	$desc_fields .= '<img class="action-text3" src="./images/del.png" alt="" onclick="if(document.mainform.desc.rows>7){document.mainform.desc.rows-=4}else{document.mainform.desc.rows-=(document.mainform.desc.rows-4)};" onmouseover="Tip(\'Remove 4 rows\')" onmouseout="UnTip()" />';
	$desc_fields .= '</dd>';
}
...
if(!empty($author))
{
	foreach($author as $key => $value)
	{
		...
		$author_fields .= '<dd class="author-rows"><label><input type="checkbox" name="author[' . $field_id . '][phpbbcom]" id="author-' . $field_id . '-phpbbcom"' . ((isset($value['phpbbcom'])) ? ' checked="checked"' : '') . ' /><span style="font-size: 12px;">(Check here if this author is not registered at phpbb.com.)</span></label></dd></dl>';
					...
					foreach($value2 as $key3 => $value3)
					{
						...
						if($value3['type'] != '')
						{
							...
							if(strpos($value3['type'], 'inline') !== FALSE)
							{
								...
								$modx_input = '<span id="' . $dd_id . '_field"><input id="' . $dd_id . '_data" name="modx[' . $file_id . '][' . $edit_id . '][' . $dl_id . '][data]" size="88" value="' . gen_value($value3['data']) . '" type="text"></span>';
								$modx_img = '<img id="' . $dd_id . '_info" class="action-arrows" src="./images/info_8.png" alt="" onmouseover="Tip(\'Note that the inline tags may not contain line breaks\')" onmouseout="UnTip()">';
							}
							else if($value3['type'] != 'comment')
							{
								...
								if($value3['type'] == 'find')
								{
									$modx_img = '<img id="' . $dd_id . '_info" class="action-arrows" src="./images/info_2.png" alt="" onmouseover="Tip(\'Find tags in the MODX file should be in the order that the find targets appear in the file. In other words, a processor of the MODX file should never need to go backwards in the file to locate all of the finds. When there are multiple finds within a single edit tag, the processor should handle all finds before any actions.\')" onmouseout="UnTip()">';
								}
								else
								{
									$modx_img = '<img id="' . $dd_id . '_info" class="action-arrows" src="./images/info_12.png" alt="" onmouseover="Tip(\'The string to add before the find, add after the find, replace the find with, or the operation string described above.&lt;br /&gt;&lt;br /&gt;The syntax for the operation action is a bit obscure because it uses tokens. The find action would have a token like this&lt;br /&gt;colspan=&quot;{:%1}&quot;&lt;br /&gt;The operation action would look like this:&lt;br /&gt;{:%1} + 1\')" onmouseout="UnTip()">';
								}
							}
							else
							{
								...
								$modx_img = '<img id="' . $dd_id . '_info" class="action-arrows" src="./images/info_17.png" alt="" onmouseover="Tip(\'Comment pertaining to this edit\')" onmouseout="UnTip()">';
							}
							...
							$modx_fields .= '<br /><img class="action-arrows" src="./images/plus_up.png" alt="" onclick="modx_add_field(\'modx[' . $file_id . '][' . $edit_id . ']\', \'' . $dl_id . '\', \'dl\', \'above\')" onmouseover="Tip(\'Add action above\')" onmouseout="UnTip()">';
							$modx_fields .= '<img class="action-arrows" src="./images/plus_down.png" alt="" onclick="modx_add_field(\'modx[' . $file_id . '][' . $edit_id . ']\', \'' . $dl_id . '\', \'dl\', \'below\')" onmouseover="Tip(\'Add action below\')" onmouseout="UnTip()">';
